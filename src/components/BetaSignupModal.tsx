...
export default function BetaSignupModal() {
  ...
  useEffect(() => { const handler = () => { setOpen(true); setSuccess(false); setError(""); }; window.addEventListener(BETA_SIGNUP_EVENT, handler); return () => window.removeEventListener(BETA_SIGNUP_EVENT, handler); }, []);
  if (!open) return null;
